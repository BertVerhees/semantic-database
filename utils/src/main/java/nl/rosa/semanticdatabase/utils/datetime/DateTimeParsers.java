...
public class DateTimeParsers {
    ...
    public static CombinedPeriodDuration parseDurationValue(String text) {
        try {
            if (text.startsWith("PT") || text.startsWith("-PT")) {
                return CombinedPeriodDuration.from(Duration.parse(text));
            } else if (text.contains("T")) {
                return CombinedPeriodDuration.from(Duration.parse(text));
            } else {
                return CombinedPeriodDuration.from(Period.parse(text));
            }
        } catch (DateTimeParseException e) {
            throw new IllegalArgumentException(e.getMessage() + ":" + text);
        }
